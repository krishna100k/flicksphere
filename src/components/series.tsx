...
const Series: React.FC<any> = ({ data, id, category }) => {
  ...
  return (
    <div className="bg-[#180E39] w-full min-h-[50rem] lg:min-h-auto lg:h-[50rem]  md:mt-20 md:mb-20 flex lg:flex-row flex-col-reverse overflow-hidden">
      <div className=" w-full min-h-[20rem] lg:w-[25rem] flex flex-col gap-5 overflow-y-scroll">
        <p className="pt-5 ml-5 text-sm">List Of Episodes</p>
        <select
          onChange={(e) => fetchData(e?.target?.value)}
          className=" w-[85%] min-h-10 rounded-sm color-black bg-[#030712] border border-slate-600 ml-5 text-sm pl-1"
          name="seasons"
        >
      ...
      <iframe
        ...
        src={`https://player.videasy.net/tv/${id}/${seasonNumberQuery}/${epNumberQuery}`}
